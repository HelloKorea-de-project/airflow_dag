...
def load_to_s3_raw(**kwargs):
    ...
    festival_data_json = ti.xcom_pull(task_ids='extract_festival_data')
    df = pd.read_json(festival_data_json)
    ...
    s3_key = f'source/kopis/festival/{kst_date.year}/{kst_date.month}/{kst_date.day}/festival_{kst_date.strftime("%Y%m%dT%H%M%S")}.csv'
...
extract_festival_data >> load_to_s3_raw