...
def get_facility_s3_key(**kwargs):
    """
...
def get_facility_detail_s3_key(**kwargs):
    """
...
def join_tables(**kwargs):
    ...
    facilities_s3_key = ti.xcom_pull(task_ids='get_facility_s3_key')
    facility_detail_s3_key = ti.xcom_pull(task_ids='get_facility_detail_s3_key')
    ...
    byte_buffer = io.BytesIO(facilities_file_obj.get()["Body"].read())
    logging.info(f"parquet schema: {pq.read_schema(byte_buffer)}")
    facilities_df = pd.read_parquet(byte_buffer)
    logging.info(f"Dataframe cols: {facilities_df.columns}")
    logging.info(f"Dataframe schema: {facilities_df.dtypes}")
    byte_buffer.seek(0)
    ...
    logging.info(f'excution date: {execution_date}')
    logging.info(f'kst_date: {kst_date}')
    s3_temp_key = f'source/kopis/facilities/{kst_date.year}/{kst_date.month}/{kst_date.day}/joined_facilities_{kst_date.strftime("%Y%m%d")}.parquet'
...
def ensure_integrity(**kwargs):
    ...
    logging.info(f'excution date: {execution_date}')
    logging.info(f'kst_date: {kst_date}')
    s3_ensured_key = f'source/kopis/facilities/{kst_date.year}/{kst_date.month}/{kst_date.day}/ensured_facilities_{kst_date.strftime("%Y%m%d")}.parquet'
...
def load_to_rds(**kwargs):
    ...
    # load to rds
    df.to_sql('tour_performancesfacilities', con=engine,if_exists='append', index=False)
...
def manage_null(df):
    ...
    df = df.fillna({'sidonm': '서울'})
    # Delete records including NULL with below columns
    df = df[['mt10id', 'fclynm', 'sidonm', 'gugunnm', 'adres', 'lo', 'la', 'createdAt', 'updatedAt']].dropna()
...
# Define the DAG
dag = DAG(
    'produce_facility_data',
    default_args=default_args,
    description='A DAG to load event facility data to rds every day',
    schedule_interval='@once', # triggered by stage_tour_data every day
    start_date=days_ago(1),
    catchup=False,
)

get_facility_s3_key = PythonOperator(
    task_id='get_facility_s3_key',
    python_callable=get_facility_s3_key,
    dag=dag,
)

get_facility_detail_s3_key = PythonOperator(
    task_id='get_facility_detail_s3_key',
    python_callable=get_facility_detail_s3_key,
    dag=dag,
)
...
[get_facility_s3_key, get_facility_detail_s3_key] >> join_tables >> ensure_integrity >> load_to_rds